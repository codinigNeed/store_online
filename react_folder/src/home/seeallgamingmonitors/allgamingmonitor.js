...
import {produitmonitor} from "../gamingMonitors/TAbleMoniture";
...
import './seeallpc.css';
export default function AllGamingMontor(){
    const dispatch = useDispatch();
    const [gimingMonitors,setgimingMonitors] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/products`);
                setgimingMonitors(response.data);
                } catch (error) {
    ...
    const  gimingMonitor = gimingMonitors.filter((t)=>{return t.categorie_id == 4})
