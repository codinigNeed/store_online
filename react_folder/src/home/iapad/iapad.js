...
import { produitiapad } from "./iapadproduit";
...
export default function Iapad(){
    const [iapads,setiapads] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/products`);
                setiapads(response.data);
                } catch (error) {
    ...
    const  iapad = iapads.filter((t)=>{return t.categorie_id == 7})
        ...
        <div className="div-imag-pc">
            <div>
                ...
                <div className="aseeAllpc">
                    <Link to={'/Alliapad'} className="see">
                    <a href="" className="seeAll">see all product</a>
                    </Link>
                </div>
            </div>
            {iapad.slice(0,4).map((item)=>{
